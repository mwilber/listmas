ggControllers.controller('ShareCtrl', ['$scope', '$rootScope', '$http', 'ggActiveList', 'ggProStatus', 
function($scope, $rootScope, $http, ggActiveList, ggProStatus) {
            ...
            $scope.db.transaction(function (tx) {
                tx.executeSql('SELECT * FROM tblShoplist WHERE tblShoplist.shoplistId=?', [ggActiveList.GetActiveList()], function(tx, result){
                    ...
                    if( result.rows.length > 0 ){
                        ...
                        $scope.list.shoplistTheme = result.rows.item(0).shoplistTheme;
                    }
    ...
    $scope.PubClose = function(){
        pubresult.hide();
        if( ggProStatus.GetProStatus() != 1 ){
            $rootScope.unlockTitle = 'Enhanced Lists';
            mpro.show('modal');
        }
    ...
    $scope.PickTheme = function(){
        ...
        if( ggProStatus.GetProStatus() == 1 ){
            ...
            $rootScope.unlockTitle = "List Theme";
            mpro.show('modal'); 
        }
    ...
    $scope.WarnXfer = function(){
        if( ggProStatus.GetProStatus() == 1 ){
            ...
            $rootScope.unlockTitle = "Transfer List";
            mpro.show('modal'); 
        }
    };
    ...
    $scope.TxShare = function(){
        
        try{
            ga('send', 'event', 'button', 'click', 'share_tx', 0);
        }catch(exception){
            console.log("ga fail");
        }
        
        var txurl = "sms:?body="+escape($scope.metadata.title)+escape(": ")+escape($scope.shareUrlRoot+$scope.list.shoplistUrl)+escape(" ")+escape($scope.metadata.description);
        if($('body').hasClass('iOS')){
            txurl = "sms:&body="+escape($scope.metadata.title)+escape(": ")+escape($scope.shareUrlRoot+$scope.list.shoplistUrl)+escape(" ")+escape($scope.metadata.description);
        }  
        window.open(txurl, '_system');
        return false;
    };
