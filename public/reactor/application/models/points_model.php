...
class Points_Model extends CI_Model
{
	var $table = "tblPoints";
	var $pk = "pointsId";
	var $ds = "pointsTimeStamp";  //Default sortby field 
	var $rq = "pointsValue";		//Required field (you'll need to mod the form validation if there isn't one)
	var $fields = array(
		 'pointsValue' => array('label'=>'Value','type'=>'int'),
		 'pointsMoneyValue' => array('label'=>'Money','type'=>'decimal','constraint'=>'10,2'),
		 'pointsType' => array('label'=>'Type','type'=>'varchar','constraint'=>10),
		 'pointsTypeRef' => array('label'=>'TypeRef','type'=>'varchar','constraint'=>10),
	...
	function GetProfileScore($pProfile){
		
		$sql = "SUM(pointsValue) AS pointsTotal, SUM(pointsMoneyValue) AS pointsMoneyTotal FROM ".$this->table." WHERE profileId=".$pProfile;
