...
{
	var $table = "tblShopList";
	var $pk = "shopListId";
	var $ds = "shopListDateAdded";  //Default sortby field
	var $rq = "shopListName";		//Required field (you'll need to mod the form validation if there isn't one)
	var $fields = array(
		 ...
		 'shopListImage' => array('label'=>'Image','type'=>'varchar','constraint'=>200),
		'shopListTheme' => array('label'=>'Theme','type'=>'varchar','constraint'=>200),
		 ...
		 'shopListEnhanced' => array('label'=>'Enhanced','type'=>'tinyint'),
		 'shopListDateAdded' => array('label'=>'Date Added','type'=>'timestamp'),
		);
	...
	function GetLists($pProfile){
		$this->db->where('profileId', $pProfile);
		$this->db->order_by('shopListCheckoff', 'ASC');
		$this->db->order_by($this->ds, 'DESC');
		$this->db->limit(LISTLIST_LIMIT);
