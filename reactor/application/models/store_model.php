...
class Store_Model extends CI_Model
{
	var $table = "tblStore";
	var $pk = "storeId";
	var $ds = "storeDateAdded";  //Default sortby field 
	var $rq = "storeName";		//Required field (you'll need to mod the form validation if there isn't one)
	var $fields = array(
		 'storeName' => array('label'=>'Name','type'=>'varchar','constraint'=>50),
		 'storeGid' => array('label'=>'gId','type'=>'varchar','constraint'=>50),
		 'storeAddress' => array('label'=>'Data','type'=>'varchar','constraint'=>500),
		 'storeLat' => array('label'=>'Lat','type'=>'float','constraint'=>'10,6'),
		 'storeLng' => array('label'=>'Lng','type'=>'float','constraint'=>'10,6'),
		);
	...
	function GetByProfile($pProfileId){
		$this->db->select('*');
		$this->db->from($this->table);
		$this->db->join('tblProfileStore', 'tblProfileStore.storeId = tblStore.storeId');
		$this->db->where('profileId',$pProfileId);
